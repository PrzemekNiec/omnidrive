...
| Z10-15 | ✅ | `e2e_recovery` i `e2e_sync` hardkodowały `Y:` i nie robiły `subst /D` w `Drop` — stąd porzucone mapowania. Skala okazała się większa: po jednym pełnym przebiegu w systemie zostawało **osiem** mapowań, w tym **`O:`**, czyli produkcyjna litera Skarbca. Montowanie dzieje się przy **każdym** `POST /api/unlock` (`api/auth.rs:115-132`), czego `OMNIDRIVE_E2E_TEST_MODE` nie wyłącza (wyłącza tylko bootstrap w `main.rs`), a `select_mount_drive_letter` przy zajętej literze preferowanej skanował `D`..`Z` **rosnąco** — w tym zakresie leży `O`. `e2e_reconciliation` i `e2e_scrubber_repair` wcale nie ustawiały `OMNIDRIVE_DRIVE_LETTER`, więc prosiły o `O:` jako preferowaną. **NAPRAWIONE** `5bba1cc`: fallback skanuje malejąco i nigdy nie zwraca `O` (preferowana `O:` w produkcji nietknięta), a testy rezerwują litery ze wspólnej puli bez `O`, pomijając litery już zajęte, i odmapowują je w teardownie. Suita 257/0, `subst` pusty | czytanie + oględziny `subst` po przebiegu |
