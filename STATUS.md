...
> **Ostatnia aktualizacja:** 2026-07-31 (**3 dekompozycje + 6 napraw z live smoke** — §12.7b i §12.7c. `db.rs`/`smart_sync.rs`/`downloader.rs` rozbite z maszynowym dowodem zero-drift; smoke wykrył 6 błędów, których nie złapała suita 204 testów, w tym P1-009 czyniący produkt niezdatnym do codziennego użytku. **Następna sesja → β.a live smoke na Dellu**, ale najpierw bump v0.3.29 + przebudowa instalatora.)
> **Aktualna wersja:** `v0.3.28` w kodzie — **bump do v0.3.29 zaległy** (6 poprawek z 2026-07-31 nieodwydane). ⚠️ Ostatni instalator `OmniDrive-Setup-0.3.28.exe` i binarki w `dist/installer/payload/` są **sprzed tych poprawek** — zawierają wszystkie 6 błędów, NIE wozić na Della. Lenovo działa z `target/release`.
> **Status:** Fazy 0/α/β/γ ZAMKNIĘTE. Dług techniczny: 3 największe monolity rozbite (§12.7b). Tracker `KNOWN_ISSUES`: P0/P1/P3 puste, w P2 zostają P2-001 (CPU watchera, pomiar PASS — kandydat do zamknięcia decyzją) i P2-002 (lagi VFS → Faza ε). Zaległe operacyjne: **Scaleway IAM** na prefiks `_omnidrive/system/*` (dzisiejsza poprawka sum kontrolnych naprawia packi, ale NIE ten prefiks) oraz **β.a live smoke na Dellu** — nadal niezweryfikowany empirycznie. *(historia poniżej)* Faza 0 ZAMKNIĘTA + **Faza α.A.a (P1-006 logout-locks-vault) DONE** — fix `api/auth.rs::post_auth_logout` woła `state.vault_keys.lock().await` przed `delete_user_session`. SMOKE H1 gate: logout=200, session=invalid, safety-numbers=401, O: not mounted — 4/4 zielone. Pre-push hook aktywny, CI z fmt-check gate. **α.A.b.3 ZAMKNIĘTE 4/4** — 3.1 scaffold (`204b287`) + 3.2 message pump (`241030b`: kanoniczne WNDCLASSW + feature `Win32_Graphics_Gdi`, `catch_unwind` trampoline, `OBSERVER_CTX` OnceLock, `WTS_SESSION_LOCK`→`force_lock_and_dismount(WinSessionLock)`, UNLOCK ignored zero-trust) + 3.3 test-helpers bridge (`72e0ed8`: `OBSERVER_HANDLE` + `test_routes`/`post_test_simulate` + e2e mpsc) + 3.4 spawn observer w `ApiServer::run` z graceful degradation (`ead8039`: cfg-windows blok, `OBSERVER_HANDLE.set`, info/warn timer-only fallback) DONE+pushed. 4 ścieżki locka (logout/idle/Win+L/manual) zbiegają się na `force_lock_and_dismount`. **α.A.b.4 frontend DONE** (`2031e85`: chip countdown + settings card inline w index.html, review-first 2 subagenci + 4 a11y fixy). **SMOKE H2 (idle) + H3 (Win+L) PASS** live na Lenovo (log-confirmed `reason=IdleTimeout` / `reason=WinSessionLock`). **Bug wykryty przez SMOKE i naprawiony** (`8e0d116`): `require_session`/`require_role` touchowały timer na każdym auth-callu → dashboard `fetchAuditLog` (30s) resetował idle → auto-lock nigdy nie odpalał; fix = idle activity tylko z realnego inputu (POST /touch) + plików (CfApi). **Faza α.A.b ZAMKNIĘTA. α.A.c ZAMKNIĘTA** — Zeroize newtype KeyBytes (P2-005), SMOKE H4 PASS (memdump po lock: `after.dmp`=0 trafień), workspace v0.3.26. **Następna sesja → α.B.a** (Argon2id 2026 params bump). Build instalatora pending.
>
...
│
✅ α — Crypto Hardening — ZAMKNIĘTA (A+B+C+D, v0.3.27)
...
│
└── D. Spec & formal review (QG5) ──────────────── ✅ ZAMKNIĘCIE FAZY α
    └── α.D.a — crypto-spec.md §12–§15 + formal Claude review (QG5)  ✅ DONE
```
...
| **α.D.a** ✅ | **crypto-spec.md update + Formal Claude crypto review (QG5).** Reconcile `crypto-spec.md` (§7.2 params α.B.a) + 4 nowe sekcje: §12 AAD semantics (P3-001), §13 auto-lock + zeroize, §14 device key-exchange (X25519 + ML-KEM-768 hybrid), §15 identity grafting. Formalny przegląd całego pipeline → `docs/superpowers/specs/2026-06-06-crypto-review.md`. | **DONE 2026-06-06** — review **QG5 PASS (warunkowy)**, zero Critical/High; 7 findings (F-1 Medium = `revoke_device` nie NULLuje hybrydowego wrapu → P2-006, nie blokuje v0.4 bo multi-device hybrid nie-live; F-2/F-3 Low → P3-003/P3-004; F-4–F-7 Info). Doc-only (zero zmian w kodzie, decyzja Przemka), bez bumpu (v0.3.27). Dokument review **zaakceptowany przez Przemka 2026-06-06 w 100%**. F-3 świadomie zostawione jako rekomendacja (rygor doc-only). **Faza α ZAMKNIĘTA.** |
...
├── γ.a — Resume upload after crash    ✅ CLOSED — MOOT (decyzja 2026-07-30) — uploader single-shot ~4MB PUT (zero multipart), crash→pack w spoolu→whole-object retry; intencja Zero-Data-Loss już spełniona, bez kodu
├── γ.b — Conflict copy                ✅ ZBUDOWANE + ZWERYFIKOWANE (γ.1, 8 testów; obnażył+naprawił root-uniqueness bug); cross-device → δ
├── γ.c — Soft-delete grace 7d + Kosz  ✅✅ DONE + LIVE SMOKE PASS (2026-07-20) — restore re-projektuje do O:\ (fix f3e9edc)
└── γ.d — Snapshot upload guard        ✅ DONE + ✅ SMOKE G2 PASS 2026-07-30 (Lenovo) — spec-premisa moot (append-only + advance-on-success); domknięte: restore-fallback, lokalny .bak/24h, sanity guard latest + 3 bugi z review (relative-path .bak, złe hasło→"brak kopii", S3 listing brał 32 NAJSTARSZE)
```

| Krok | Zakres | Stan (audyt 2026-07-20) |
|------|--------|-----|
| **γ.a** ✅ | **Resume upload after crash.** Roadmapa: multipart state w SQLite (`multipart_uploads`), wznów pending parts. | ✅ **CLOSED — MOOT (decyzja Przemka 2026-07-30, code-grounded).** `uploader.rs:293` = single-shot `put_object` całego pliku, packi ~4MB (BEZ multipart; grep całego repo: zero `create_multipart_upload`/`upload_part`). Istnieje tylko `cleanup_stale_multipart_uploads` (abort porzuconych, `onboarding.rs`) — defensywa, NIE resume. Crash mid-upload → atomowy fail PUT → pack zostaje w spoolu → upload worker retry’uje **cały** obiekt (retry-storm plateau/permanent z β). **Werdykt: spool-persist + whole-object retry już spełniają intencję Zero-Data-Loss; multipart-resume bezprzedmiotowy.** Follow-up (osobny, NIE γ.a): spot-check że spool nie jest kasowany przed potwierdzonym uploadem. **Faza γ ZAMKNIĘTA W CAŁOŚCI (γ.a moot, γ.b/c/d DONE).** |
| **γ.b** ✅ | **Conflict copy.** 2-device → 2 revisions materialized w O:\. | ✅ **ZBUDOWANE + ZWERYFIKOWANE (γ.1 2026-07-20).** `watcher.rs:424` → `packer.pack_file_with_expected_parent` → `db::classify_revision_lineage` (4-way) → `materialize_conflict_copy_from_revision` → `conflict_events` → surface `list_recent_conflicts` (Multi-Device tab). **8 testów** (4 lineage + 2 materializacja + 1 root-uniqueness + 1 packer wiring), commity `9b53132`/`65b2eb9`/`5e97700`. **🐛 Test obnażył realny bug** (dezambiguacja nazwy kopii martwa w root vaulta — SQLite NULL≠NULL w `UNIQUE(parent_id,name)`) → **naprawiony** expression unique index `COALESCE(parent_id,-1),name` (`65b2eb9`, additive, bez zmiany modelu; egzekwuje też root uniqueness dla WSZYSTKICH plików). Suite angeld **167** green. Cross-device (2 fizyczne urządzenia) = Faza δ (β.b roster-merge-only nie aplikuje rewizji plików). |
| **γ.c** ✅ | **Soft-delete grace period.** `inodes.deleted_at` + grace 7 dni + UI „Kosz". Twardy delete po grace. | ✅ **KOD DONE 2026-07-20.** Spec `docs/superpowers/specs/2026-07-20-gamma-soft-delete-design.md`, plan `…/plans/2026-07-20-gamma-soft-delete.md`. In-place `inodes.deleted_at`, oba triggery (watcher `handle_deleted_path` + API `delete_file` → `soft_delete_inode`), listowania/resolve filtrują `deleted_at IS NULL` (raw-by-id niefiltrowany), Kosz queries (list/restore z disambiguacją/expired), sweeper 1h w main.rs, API `/api/trash` (list/restore/purge, ACL Member), widok Kosz w SPA (lista wierszy, badge dni, empty state). **🐛 Test obnażył interakcję z γ.1 indexem** (liczył soft-deleted → blokował nowy plik o tej samej nazwie) → fix **partial index** `WHERE deleted_at IS NULL`. 8 testów (schema/soft-delete/blast-radius/Kosz-queries/sweeper), suite angeld **174** green, commity `2dfbff5..7b8dae1`. **✅✅ LIVE SMOKE PASS 2026-07-20** (Lenovo, real vault): delete w Eksploratorze → `watcher soft-deleted inode` → widoczny w Koszu z badge → Przywróć czyści `deleted_at` + **re-projektuje plik do O:\** (fizycznie potwierdzone). Smoke wykrył lukę re-projekcji → fix `f3e9edc` (`project_vault_to_sync_root` po restore) + kosmetyk ikony `7b8dae1`. Migracja schematu (deleted_at + partial index) wykonała się na produkcyjnej bazie bez błędu. Cross-device/katalogi/bulk/konfig-grace = poza zakresem. |
| **γ.d** | **Snapshot upload guard.** Nie nadpisuj dobrego snapshotu przy all-provider-fail; lokalny backup. | ✅ **DONE 2026-07-21.** Spec `docs/superpowers/specs/2026-07-21-gamma-d-snapshot-guard-design.md`, plan `…/plans/2026-07-21-gamma-d-snapshot-guard.md`. Premisa specowa („nie nadpisuj dobrego snapshotu") potwierdzona jako **już spełniona strukturalnie**: klucz timestampowany (append-only), `latest.db.enc` awansuje wyłącznie po udanym uploadzie snapshotu, 0 sukcesów → `NoSuccessfulUploads` bez przesunięcia markera. Audyt wyciągnął 3 inne luki i te zostały domknięte: **G1** `restore_metadata_from_cloud` przerywał całe odzyskiwanie na pierwszym nieodszyfrowywalnym obiekcie (a `latest.db.enc` jest kandydatem pierwszym) → teraz `continue` + cache derywacji Argon2id per parametry KDF; **G2** periodyczny `omnidrive.db.bak.YYYYMMDD_HHMMSS` co 24h z retencją 3, wykonywany PRZED `require_master_key` (działa przy zalockowanym vaulcie, gdy snapshot chmurowy w ogóle nie powstaje), rotacja ignoruje ręczne `.bak.preSmoke-*`; **G3** sanity guard — brak `vault_state`/`vault_config` albo spadek `inodes`/`data_encryption_keys` do zera przy niezerowym baseline blokuje wyłącznie awans `latest`, timestampowany snapshot leci normalnie, baseline w `system_config` nie przesuwa się do czasu powrotu liczników. **🐛 Finalny review całej gałęzi (Opus) złapał 3 realne bugi, wszystkie naprawione:** (1) **C1** `.bak` był no-opem przy WZGLĘDNEJ ścieżce bazy — `.parent()` daje `Some("")`, nie `None`, więc guard nie wchodził i `read_dir("")` błądził; repo ma `OMNIDRIVE_DB_URL=sqlite://omnidrive.db`, czyli na Lenovo nie powstałaby ani jedna kopia (`backup_parent_dir` normalizuje pusty parent na `.`; ta sama normalizacja propagowana do `runtime_paths::sqlite_db_directory`); (2) **C2** po G1 funkcja nigdy nie zwracała `BackupDecryptFailed`, więc gałąź `→ IncorrectPassphrase` w `onboarding.rs` stała się martwa, a substring-matching łapał zawsze obecny `latest.db.enc` → **złe hasło w kreatorze DR raportowane jako „nie znaleziono kopii"**, przy okazji do 33 pełnych pobrań na providera (a `maybe_auto_restore_database` woła z `pool: None`, czyli BEZ licznika egressu). Fix: licznik wyłącznie błędów AEAD, stop po 3, końcowy błąd `BackupDecryptFailed` — odzyskiwanie mimo 2 uszkodzonych obiektów zachowane; (3) **I1 (pre-existing)** `list_objects_v2().max_keys(32)` zwracał 32 **NAJSTARSZE** snapshoty (S3 sortuje rosnąco, MaxKeys ucina głowę) → po ~miesiącu fallback w chmurze wskazywał snapshoty z pierwszego miesiąca życia vaulta; teraz paginacja po `continuation_token` (cap 20 stron) + wspólny helper `newest_snapshot_keys` dla obu implementacji. 12 testów, suite angeld **186** green, commity `b508f7b..9832783`. Nowe testy stopu i AEAD-fallbacku zweryfikowane mutacyjnie (podniesienie/obniżenie limitu → czerwone). **✅ SMOKE G2 PASS 2026-07-30** (Lenovo, daemon z `target/release`, real dev vault): (1) `.bak.20260730_202024` utworzony ~25 ms po starcie PRZY ZALOCKOWANYM vaulcie — potwierdza fix C1 (względna ścieżka `.` zadziałała, wcześniej no-op); (2) ręczne `.bak.preSmoke-*`/`.preCleanup-*` nietknięte (rotacja je pominęła); (3) po odblokowaniu wymuszony snapshot — **sanity guard NIE załączył się** (zdrowa baza), snapshot uploaded (B2 COMPLETED), `latest.db.enc` awansował; R2 ConnReset 10054 + Scaleway 120 s timeout = znane P1-004/003, ortogonalne do γ.d. Smoke przy okazji wykrył 3 bugi UI w `index.html` (przycisk backup-now → 404 zły prefiks `/api/maintenance/`; druk `window.open(noopener)`→`null`=puste okno; CTA „Weryfikuj teraz" odpalał regenerację klucza) — wszystkie naprawione. |
...
| **P2-008** ✅ | **Dekompozycja `smart_sync.rs`** (2 236 linii: warstwa publiczna + `mod imp` cfapi 1 940). Audyt §2.2 = „clean split candidate, risk zero". | ✅ **DONE 2026-07-31**, commit `d0f7876`. `smart_sync/mod.rs` (298, API bez zmian) + `imp/` × 7: `registration` 622, `callbacks` 480, `placeholder` 340, `projection` 321, `paths` 148, `state` 86, `lifecycle` 86. **Korekta oceny audytu:** „risk zero" niedoszacowane — podział wymusił `pub(super)` na **30 elementach + 13 polach struktur** (widoczność wyliczona z referencji + domknięcie przechodnie na typy w podniesionych sygnaturach). Zero-drift: **100 bloków, 97 bajt-w-bajt**, 3 różne wyłącznie łamaniem linii po zmniejszeniu wcięcia — dla nich kryterium = wynik identyczny z `rustfmt(baseline)`. **⚠️ Moduł nie ma testów jednostkowych; poprawność runtime cfapi weryfikuje wyłącznie live smoke — nieprzeprowadzony.** |
...
### 12.7c Live smoke 2026-07-31 — **6 napraw z jednego uruchomienia** *(między γ a δ)*

> **Kontekst:** po trzech dekompozycjach (§12.7b) uruchomiono daemona z `target/release` na realnym dev-vaulcie, żeby potwierdzić, że refaktory nie zepsuły runtime'u. Refaktory były czyste — ale smoke wykrył **sześć niezwiązanych z nimi błędów**, z czego dwa czyniły produkt niezdatnym do normalnego użytku.
>
> **⚠️ Żadnego z tych sześciu nie złapała suita 204 testów jednostkowych.** Wszystkie siedziały w miejscach, do których ta suita nie sięga: integracja z Windows (cfapi), pętle workerów, sekwencje lock/unlock, konfiguracja klienta S3.

| ID | Rzecz | Stan |
|----|-------|------|
| **P1-009** ✅ | **Odblokowanie nie zerowało licznika bezczynności** → po przerwie >5 min skarbiec lockował się sekundę po odblokowaniu, `O:` znikał. | ✅ **FIXED** `8097327`. Pętla auto-locka pomija zablokowany skarbiec, więc `last_activity` było zamrożone; licznik odświeżały tylko uwierzytelnione żądania i cfapi, a unlock idzie **bez sesji**. Fix: `auto_lock::touch` w obu ścieżkach (hasło + Windows Hello). **Auto-lock wszedł w v0.3.25 → wada żyła ~2 miesiące**, bo w cyklu dev odblokowuje się zaraz po starcie daemona (jedyny działający przypadek). Live: 19 min bezczynności → unlock → 0 auto-locków, `O:` żyje >90 s. |
| **P1-007** ✅ | **Jeden zepsuty provider blokował całą kolejkę uploadu** (do ~37 dni). | ✅ **FIXED** `0d57c82`. `ORDER BY id ASC` + requeue z tym samym id + `sleep(delay)` w pętli **jedynego** workera = głodzenie wszystkich kolejnych zadań. Fix: `upload_jobs.next_attempt_at` (migracja addytywna), odroczenie w bazie zamiast snu w pętli. **+4 testy, zweryfikowane mutacyjnie.** Live: job #7 odroczony o 16:52:46.548, job #8 ruszył 16 ms później. |
| **P1-003/004** ✅ | **KOREKTA DIAGNOZY** — R2 `ConnectionReset` i Scaleway timeout to **nie** sieć ani IAM. | ✅ **FIXED** `338e641`. `aws-sdk-s3` 1.119 ma domyślnie `request_checksum_calculation=WhenSupported`; dla ciała **strumieniowego** dokleja CRC32 w `aws-chunked` z trailerem — R2 zrywa połączenie, Scaleway czeka do timeoutu, **B2 toleruje i maskował problem**. Dowód: ten sam shard, ta sama minuta — `repair.rs` (bufor) wysyła w 0,8 s, `uploader.rs` (strumień) pada. Czerwcowe „naprawy" (`pool_idle_timeout`, retry) leczyły objaw. Fix: `WhenRequired`, 7 linii + test konfiguracji. |
| **P1-008** ✅ | **Placeholder serwował starą wersję pliku** po aktualizacji. | ✅ **FIXED** `0228239`. Placeholder trzyma `(inode, revision)` w `FileIdentity`; `create_projection_placeholder` miał wczesne wyjście `if !exists`, więc dla istniejącego pliku nie robił nic. Baza znała nową rewizję → rozjazd niewidoczny z poziomu danych. Fix: `update_placeholder_revision` (`CfUpdatePlaceholder` + DEHYDRATE + MARK_IN_SYNC), tylko przy realnej zmianie rewizji (bezwarunkowo = niepotrzebny egress). Naprawione w 3 miejscach. |
| **P2-010** ✅ | **Watcher przepakowywał niezmienione pliki co 30 s.** | ✅ **FIXED** `e371ca8`. `scan_existing_files` tworzył własną pustą `processed_files` → `previous_state` zawsze `None` → dedup po rozmiarze/mtime **i** po SHA-256 martwy. Koszt: nowa rewizja + pełny hash + ponowne szyfrowanie co tik (jeden plik uzbierał **1425 rewizji**). Live A/B: 3 min bez zmian → 0 nowych rewizji (było ~6); realna zmiana → dokładnie 1. |
| **P2-011** ✅ | **Błąd jednego pliku blokował montowanie całego skarbca.** | ✅ **FIXED** `b892b23`. W `project_vault_to_sync_root` błąd szedł przez `?` i przerywał projekcję wszystkich. Fix: log + pomiń, reszta się montuje; trzy fatalne kroki niosą teraz kontekst (nazwa kroku + ścieżka) zamiast gołego kodu Windows. |

**Ślepe zaułki warte zapamiętania:** `NOT_A_CLOUD_FILE` (0x80070178) i `CfConnectSyncRoot` (0x80070186) wyglądały na przyczynę niezamontowanego `O:` — były **skutkiem** wyścigu z demontażem od auto-locka (P1-009). Diagnoza dwukrotnie poszła przez nie w złą stronę. Przy sumach kontrolnych spalone dwie hipotezy (brak `SizeHint`, adapter `http-body` 1.x) — obie wycofane z kodu.

**⚠️ 4 z 6 napraw nie mają testu regresyjnego** (cfapi, cykl życia stanu w workerze, handler wymagający pełnego stanu aplikacji). Przy każdej zapisany powód w `KNOWN_ISSUES` — świadoma decyzja, nie przeoczenie.

**Wniosek procesowy:** jedyne, co realnie weryfikuje ten produkt, to jego uruchomienie. Smoke ma być rytuałem po każdej zmianie dotykającej cfapi/workerów/lock-unlock, a nie wydarzeniem raz na kilka tygodni. Scenariusz **„unlock po ≥6 min bezczynności"** jest obowiązkowy — bez niego P1-009 przechodzi niezauważony.
